...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const pwd of passwords) {
      ...
      if (match) {
        isValid = true;
        matchedPasswordId = pwd.id;
        break;
      }
    }

    if (isValid && matchedPasswordId) {
...
// GET /api/verify-project-password?project_slug=xxx&password_id=xxx - Validate if specific password still exists
export async function GET(request: NextRequest) {
  try {
    ...
    // If password_id is provided, validate if it still exists
    if (passwordId) {
      const { data: password, error: passwordError } = await supabaseServiceRole
        .from("project_passwords")
        .select("id")
        .eq("id", passwordId)
        .eq("project_id", project.id)
    ...
    // Check if project has any passwords
